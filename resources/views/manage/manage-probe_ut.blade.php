...
@section('title') Probe - UT @endsection
@section('css')
...
@slot('title') Probe - UT @endslot
@endcomponent

@include('modals.probe_ut_modal')
@include('modals.modals-pending.pending_grn_for_probe_ut')

<div class="row">
    <div class="col-lg-12">
        <div class="card">
           <div class="card-header d-flex align-items-center pb-2">
                <h5 class="card-title mb-0 flex-grow-1">Probe - UT</h5>
                <div>
                    @if(hasAccess("probe_ut","export"))
                        <a href="javascript:void(0)" class="btn btn-primary" id="export-excel">Excel</a>
                    @endif

                    @if(hasAccess("probe_ut","add"))
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ProbeUtModal">Add</button>
                    @endif
                </div>
            </div>
            <div class="card-body">
...
@section('script-manage')
     <script>
        ...
        var table = $('#dyntable').DataTable({
            ...
            buttons: [{
                extend:'excel',
                filename: 'Probe-UT List',
                title:"",
                className: 'export_probe_ut d-none',
                exportOptions: { columns: ':not(:eq(0))', modifier: { page: 'all' } },
                action: newexportaction
            }],
            ajax: {
                url: "listing-probe_ut",
                type: "POST",
                headers: headerOpt,
            ...
            columns: [
                ...
                { data: 'table_unique_id', name: 'probe_ut.table_unique_id', },
                // { data: 'item_name', name: 'item.item_name', },
                { data: 'pu_probe', name: 'probe_ut.pu_probe', },
                { data: 'pu_serial_number', name: 'probe_ut.pu_serial_number', },
                ...
                { data: 'created_on', name: 'probe_ut.created_on', },
            ],
        });
