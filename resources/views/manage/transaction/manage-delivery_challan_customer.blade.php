...
@section('title') Delivery Challan (Customer) @endsection
@section('css')
...
@slot('title')Delivery Challan (Customer) @endslot
@endcomponent

@include('modals.modals-transaction.delivery_challan_customer_modal')
@include('modals.modals-details.delivery_challan_customer_details_modal')

<div class="row">
    <div class="col-lg-12">
        <div class="card">
           <div class="card-header d-flex align-items-center pb-2">
                <h5 class="card-title mb-0 flex-grow-1">Delivery Challan (Customer)</h5>
                <div>
                    @if(hasAccess("delivery_challan_customer","export"))
                        <a href="javascript:void(0)" class="btn btn-primary" id="export-excel">Excel</a>
                    @endif

                    @if(hasAccess("delivery_challan_customer","add"))
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#DcCustomerModal">Add</button>
                    @endif
                </div>
            </div>
            <div class="card-body">
...
@section('script-manage')
     <script>
        ...
        var table = $('#dyntable').DataTable({
            ...
            buttons: [{
                extend:'excel',
                filename: 'Delivery Challan (Customer) List',
                title:"",
            ...
            ajax: {
                url: "listing-delivery_challan_customer",
                type: "POST",
                headers: headerOpt,
            ...
            columns: [
                ...
                { data: 'last_on', name: 'delivery_challan_customer.last_on', },
                { data: 'created_by', name: 'created_by', },
                { data: 'created_on', name: 'delivery_challan_customer.created_on', },
        ...
        jQuery('#dyntable tbody').on('click', '.remove-item-btn', function () {
            ...
            toastDelete("Do you want to delete this record?", function () {
                jQuery.ajax({
                    url: "delete-delivery_challan_customer",
                    type: 'GET',
                    data: "id=" + data["dc_id"],
                    headers: headerOpt,
                    dataType: 'json',
